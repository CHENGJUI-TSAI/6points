...
class AgilityAnalysisSystem {
    ...
    // 新增資料
    addData(newData) {
        // Ensure a stable, unique string ID to avoid collisions when adding multiple rows quickly
        if (!newData.id) {
            newData.id = Date.now().toString() + '-' + Math.random().toString(36).slice(2,9);
        } else {
            newData.id = String(newData.id);
        }
    ...
    deleteData(id) {
        if (confirm('確定要刪除這筆資料嗎？')) {
            // Support id as number or string (from onclick attribute)
    ...
    deleteSelectedData() {
        ...
        // Use string comparison for IDs to avoid type/coercion issues
        const selectedIds = Array.from(checkedCheckboxes).map(cb => String(cb.value));
        const confirmMessage = `確定要刪除選中的 ${selectedIds.length} 筆資料嗎？`;

        if (confirm(confirmMessage)) {
            this.data = this.data.filter(item => !selectedIds.includes(String(item.id)));
            this.saveData();
            this.displayData();
            this.updateCharts();
